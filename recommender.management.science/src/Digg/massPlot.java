...
public class massPlot extends ApplicationFrame
{
    
    private static String FILE_PATH = 
	    "C" + ":" + File.separator + "academic" + File.separator+"recommender system 2"+File.separator
	    +"Management Science"+File.separator + "mass-clicks" + File.separator;
	...
	try
	{
	    BufferedWriter out = new BufferedWriter(new FileWriter(FILE_PATH + "mass_complete.csv"));
		...
		for(diggRecord dg : markedData)
		{
		    if(ids.get(i).compareTo(dg.getId()) == 0)
		    {
			int adjustcount = dg.getcumcount();
	...
	try
	{
	    //BufferedReader in = new BufferedReader(new FileReader(FILE_PATH + "mass_sorted.csv"));
	    BufferedReader in = new BufferedReader(new FileReader(FILE_PATH + "mass_complete.csv"));
